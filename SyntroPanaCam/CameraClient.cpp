...
void CameraClient::newStream()
{
	...
	settings->beginGroup(SYNTROPANACAM_STREAM_GROUP);

	if (!settings->contains(SYNTROPANACAM_HIGHRATE_VIDEO_MININTERVAL))
		settings->setValue(SYNTROPANACAM_HIGHRATE_VIDEO_MININTERVAL, "100");

	if (!settings->contains(SYNTROPANACAM_HIGHRATE_VIDEO_MAXINTERVAL))
		settings->setValue(SYNTROPANACAM_HIGHRATE_VIDEO_MAXINTERVAL, "10000");

	if (!settings->contains(SYNTROPANACAM_HIGHRATE_VIDEO_NULLINTERVAL))
		settings->setValue(SYNTROPANACAM_HIGHRATE_VIDEO_NULLINTERVAL, "2000");

	if (!settings->contains(SYNTROPANACAM_LOWRATE_VIDEO_MININTERVAL))
		settings->setValue(SYNTROPANACAM_LOWRATE_VIDEO_MININTERVAL, "6000");

	if (!settings->contains(SYNTROPANACAM_LOWRATE_VIDEO_MAXINTERVAL))
		settings->setValue(SYNTROPANACAM_LOWRATE_VIDEO_MAXINTERVAL, "60000");

	if (!settings->contains(SYNTROPANACAM_LOWRATE_VIDEO_NULLINTERVAL))
		settings->setValue(SYNTROPANACAM_LOWRATE_VIDEO_NULLINTERVAL, "6000");

	settings->endGroup();
	settings->beginGroup(SYNTROPANACAM_MOTION_GROUP);

	if (!settings->contains(SYNTROPANACAM_MOTION_MIN_DELTA))
		settings->setValue(SYNTROPANACAM_MOTION_MIN_DELTA, "500");

	   if (!settings->contains(SYNTROPANACAM_MOTION_DELTA_INTERVAL))
        settings->setValue(SYNTROPANACAM_MOTION_DELTA_INTERVAL, "200");

    if (!settings->contains(SYNTROPANACAM_MOTION_PREROLL))
        settings->setValue(SYNTROPANACAM_MOTION_PREROLL, "2000");

    if (!settings->contains(SYNTROPANACAM_MOTION_POSTROLL))
        settings->setValue(SYNTROPANACAM_MOTION_POSTROLL, "2000");
	...
	m_highRateMinInterval = settings->value(SYNTROPANACAM_HIGHRATE_VIDEO_MININTERVAL).toInt();
	m_highRateFullFrameMaxInterval = settings->value(SYNTROPANACAM_HIGHRATE_VIDEO_MAXINTERVAL).toInt();
	m_highRateNullFrameInterval = settings->value(SYNTROPANACAM_HIGHRATE_VIDEO_NULLINTERVAL).toInt();

	m_lowRateMinInterval = settings->value(SYNTROPANACAM_LOWRATE_VIDEO_MININTERVAL).toInt();
	m_lowRateFullFrameMaxInterval = settings->value(SYNTROPANACAM_LOWRATE_VIDEO_MAXINTERVAL).toInt();
	m_lowRateNullFrameInterval = settings->value(SYNTROPANACAM_LOWRATE_VIDEO_NULLINTERVAL).toInt();

	settings->endGroup();

	// get the motion stuff

	settings->beginGroup(SYNTROPANACAM_MOTION_GROUP);

	m_minDelta = settings->value(SYNTROPANACAM_MOTION_MIN_DELTA).toInt();

    m_deltaInterval = settings->value(SYNTROPANACAM_MOTION_DELTA_INTERVAL).toInt();
    m_preroll = settings->value(SYNTROPANACAM_MOTION_PREROLL).toInt();
    m_postroll = settings->value(SYNTROPANACAM_MOTION_POSTROLL).toInt();
